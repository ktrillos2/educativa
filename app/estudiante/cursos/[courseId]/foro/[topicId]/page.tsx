...
export default async function TemaForoCursoPage(props: { params: Promise<{ courseId: string, topicId: string }> }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12 max-w-4xl mx-auto">
      <Link href={`/estudiante/cursos/${params.courseId}/foro`} className="inline-flex items-center gap-1 text-[oklch(0.55_0.04_145)] hover:text-[oklch(0.35_0.10_145)] text-sm mb-2 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Volver al foro del curso
      </Link>
      ...
      <div className="bg-white rounded-xl border border-[oklch(0.88_0.04_145)] shadow-sm overflow-hidden">
        <div className="p-6">
            <div className="flex justify-between items-start gap-4 mb-4">
                ...
                {canDeleteTopic && (
                    <DeleteForumAction id={topic.id} type="TOPIC" courseId={params.courseId} />
                )}
            </div>
            ...
            return (
                <div key={reply.id} className="bg-white rounded-lg border border-[oklch(0.90_0.02_145)] p-5 shadow-sm">
                    <div className="flex justify-between items-start gap-4 mb-3">
                        ...
                        {canDeleteReply && (
                            <DeleteForumAction id={reply.id} type="REPLY" topicId={topic.id} courseId={params.courseId} />
                        )}
                    </div>
