...
export default async function AulaPage(props: { params: Promise<{ courseId: string }> }) {
  ...
  if (!enrollment) redirect(`/diplomados/${course.id}`)

  const groupId = enrollment.group_id

  // 3. Fetch live classes for this group
  let liveClasses: any[] = []
  if (groupId) {
  ...
  const groupName = Array.isArray(enrollment.course_groups)
      ...
      <div className="mt-12">
        ...
        <div className="space-y-6">
          {courseModules.map((mod, index) => {
            ...
            return (
              <div key={mod.id} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
